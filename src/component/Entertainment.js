...
function Entertainment({searchQuery}) {
  ...
  const fetchMoreListItems = async () => {
    const nextPageUrl = `https://newsdata.io/api/1/news?apikey=${apiKeys[0]}&category=entertainment&country=in&language=en&page=${nextid}`;
    await fetch(nextPageUrl)
      ...
      .then((data) => {
        ...
        (err) => {
          fetch(`https://newsdata.io/api/1/news?apikey=${apiKeys[1]}&category=entertainment&language=en&country=in&page=${nextid}`)
            .then(response => response.json())
            .then((data) => {
              setNews((prevNews) => [...prevNews, ...data.results]);
              setNextid(data.nextPage);
  ...
  useEffect(() => {
    fetch(`https://newsdata.io/api/1/news?apikey=${apiKeys[0]}&country=in&language=en&category=entertainment`)
      .then(response => response.json())
      .then((data) => {
        console.log(data, 'data');
        ...
        (err) => {
          fetch(`https://newsdata.io/api/1/news?apikey=${apiKeys[1]}&country=in&language=en&category=entertainment`)
            .then(response => response.json())
            .then((data) => {
              console.log(data, 'data');
              setNews(data.results);
